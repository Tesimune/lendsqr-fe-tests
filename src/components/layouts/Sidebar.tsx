...
const SidebarItem = ({label, icon:Icon}: any) => {
  return(
    <li>
      <a href="#/" className="flex items-center gap-3 text-sm p-5 py-3 hover:bg-secondary/10 hover:border-l-4 hover:border-l-primary">
        <Icon />
        <span>{label}</span>
      </a>
    </li>
...
const SidebarItems = ({children, label}: any) => {
  return (
    <>
      <li className="px-5 pt-3 pb-1 text-sm uppercase">{label}</li>
      {children}
    </>
  )
}
